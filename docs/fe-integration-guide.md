...
`provider=FALLBACK_STRAIGHT_LINE`이면 Kakao Mobility 응답이 아니라 좌표 기반 추정값입니다. 화면에서는 확정 경로가 아니라 예상값으로 표현하는 편이 안전합니다. 더 자세한 계산 기준은 [일정 경로 계산 설계](schedule-route-planner.md)를 확인합니다.

...
`participatingRoomCount`는 사용자가 방장 또는 승인된 멤버로 참여 중인 방 수입니다. `pendingRoomCount`는 사용자가 입장 신청 후 승인 대기 중인 방 수입니다.
...
`GET /api/me/rooms`는 사용자가 만든 방, 승인된 방, 신청 대기 중인 방을 한 번에 조회합니다.

```http
GET /api/me/rooms?tab=all
Authorization: Bearer <jwt>
```

`tab`은 생략할 수 있고, 아래 값을 사용할 수 있습니다.

| tab | 의미 |
| --- | --- |
| `all` 또는 생략 | 만든 방, 참여 중인 방, 대기 중인 방 전체 |
| `hosted` | 내가 만든 방 |
| `joined` | 승인되어 참여 중인 방 |
| `pending` | 입장 신청 후 승인 대기 중인 방 |

응답 item은 CB-06 카드 렌더링에 필요한 값을 포함합니다.
...
```json
{
  ...
  "concertStartAt": "2026-07-05T19:00:00+09:00",
  "daysUntilConcert": 14,
  "venueName": "KSPO Dome",
...
`pendingJoinRequestCount`는 방장 카드에서 승인 대기 수를 표시하기 위한 값입니다. 멤버 또는 신청 대기 카드에서는 `0`으로 내려옵니다.

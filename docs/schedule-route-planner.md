...
4. FE는 `customStartAt`과 `targetArrivalAt`을 포함한 draft 요청을 보내고, BE는 각 route segment의 거리와 소요 시간을 계산한다.
...
택시요금과 통행료는 와이어프레임과 API 명세에서 사용하지 않으므로 public response에 노출하지 않는다.
...
| 모드 | 거리 계산 | 시간 계산 |
| --- | --- | --- |
| `WALK` | 좌표 직선거리 * 1.25 | 60m/분(약 3.6km/h) 기준 올림 |
| `CAR_TAXI` | 좌표 직선거리 * 1.35 | 평균 22km/h 기준 올림 |
